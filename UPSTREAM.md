...
| Hardware (lg75, 2026-09-05, build `4a2f757`) | **Launch, UI, typing and second launch pass; playback rebooted the TV.** Installed beside the 1.x app under the new id (fresh storage, so no sign-in). Welcome screen, guest home and search render; the virtual keyboard typed the test video id through webos-webui, so SDL keyboard input works; HOME and relaunch resumed the same process (freeze request, `SIGCONT` caught, state back to Started), so upstream's resume fix holds on webOS 6.5. Playing `8gKJ9mMPuIQ` showed "This video format is not supported" with no `SbPlayer`/Starfish line ever logged, although `IsTypeSupported` had answered "probably" for VP9 up to 2160p60; about a minute later the whole TV rebooted (uptime reset, `/tmp` log lost, load average 33 just before). Same signature as upstream #68's VP9 report. The app was removed from lg75 afterwards. Quality ladder and shortcuts untested. Next: reproduce on lg48 with the Starfish decoder's log level raised, and try an H.264-only video first. |
...
|---|---|---|---|
| `v2` branch `4a2f757` (not a release) | 2026-09-05 | lg75 | Starterless build: launch, guest UI, keyboard typing and second launch OK; first playback attempt failed and rebooted the TV. Removed after the test. |
