...
| Hardware (lg75, 2026-09-05, build `4a2f757`) | **Launch, UI, typing and second launch pass; playback fails.** Installed beside the 1.x app under the new id (fresh storage, so no sign-in). Welcome screen, guest home and search render; the virtual keyboard typed the test video id through webos-webui, so SDL keyboard input works; HOME and relaunch resumed the same process (freeze request, `SIGCONT` caught, state back to Started), so upstream's resume fix holds on webOS 6.5. Playing `8gKJ9mMPuIQ` showed "This video format is not supported" with no `SbPlayer`/Starfish line ever logged, although `IsTypeSupported` had answered "probably" for VP9 up to 2160p60. The TV went down a minute later; that was the user switching it off, not the app (first recorded here as a reboot, corrected the same night). Quality ladder and shortcuts untested. Investigation continued the same night, see the rows below. |
...
|---|---|---|---|
| `v2` branch `4a2f757` (not a release) | 2026-09-05 | lg75 | Starterless build: launch, guest UI, keyboard typing and second launch OK; playback fails with "format not supported". (The TV outage during the test was a manual power-off, not a crash.) |
