...
| Fork edits on the branch | App id `com.cobalt.youtube.adfree.v2` in appinfo, so it installs beside the 1.x app and never replaces stock YouTube. `package-starterless-cobalt.sh`: `STARTERLESS_APP_ID` and `STARTERLESS_APP_TITLE` package the same build under another id; hardware tests use `com.cobalt.youtube.adfree.debug`, which takes over the 1.x debug app and its signed-in storage. `starfish_av_components.cc`: the shared backend sends `APPID` like the old decoder instead of a literal `youtube.leanback.v4` (SAM exports `APPID`, measured on lg75 2026-09-05). `fork-webos-vp9-only.patch`: AV1 hidden unless `/tmp/ytaf-av1.enable` starts with `1`, read once per process (see the AV1 row); `/tmp` is cleared on reboot, so the safe default comes back. Debug package: `main.cc` appends the switches listed in a `switches` file beside the binary (as the 1.x starter did), `STARTERLESS_SWITCHES=starterless-cobalt/debug.switches` packages one that opens devtools on `0.0.0.0:9222`, and such a package keeps `content/web/debug_remote/` for the `/json` discovery `tools/tv-*.sh` probe. `tools/tv-install-keep-login.sh` installs a 1.x or v2 IPK over an app while keeping its login (also the way back to 1.x); `tools/tv-codec.sh` reports the playing codec and the runtime's 4K codec answers. `build-dav1d-webos-docker.sh`: containers run as the host user, kept beside upstream's host-side `mkdir`. JSON.parse: upstream's preload turns `JSON.parse` into an accessor that holds one downstream parser, so the fork's own assignment replaced adblock.js's ad filter (node simulation: chain `["fork"]`). The fork filters now run from inside adblock.js's wrapper (`fork/parse-hook.js`, one `fork:` call): chain `["adblock","fork"]`. Dropped on 2026-09-23: our SDL cmake flags and codec build order, which upstream now carries. |
...
| Way back to 1.x debug | `tools/tv-install-keep-login.sh <tv> <ipk>` with the 1.x debug IPK each TV had before v2 took over the debug id: lg75 ran debug 1.2.1 (`output/com.cobalt.youtube.adfree.debug_1.2.1_arm.ipk`, byte-identical to the `v1.2.1-debug` prerelease asset), lg48 ran debug 0.0.52 (`output/com.cobalt.youtube.adfree.debug_0.0.52_arm.ipk`, local only). The login taken over from 1.x is also kept in `/media/developer/ytaf-storage-backup/debug-1x-2026-09-23/` on each TV. |
| Hardware, `a765486` merge | Not run yet. Plan: the debug-id build on lg48 first, then lg75, uninstalling the 1.x debug app first (installing 2.x over 1.x crash-loops, their #2) and restoring its login from the backup to `<app dir>/.starboard.<b64 origin>.storage` (owner `0:5000`, mode 660). The storage file lives in `$HOME`, which is the app directory, not in `content/` as with the 1.x starter. |
...
| AV1 advertised on every webOS set (upstream's capability patch) | **Hidden on `v2`** (`fork-webos-vp9-only.patch`) | AV1 through Starfish stalls on lg75 (lm21a), and the TV went down once during that stall. Upstream C1 (O20N) owners report AV1 playing, 2160p AV1+Opus on the shared backend. One binary serves both TVs, so the hide also costs lg48 AV1. | A/B per TV with the marker, no rebuild: first whether 1.x plays AV1 on each TV (`tools/tv-codec.sh` on the 1.x debug app), then v2 with the marker on the shared backend. If 1.x plays AV1 on lg75, capture the Starfish load request LG's starter sends and port the difference. |
