...
| What the merge took | 24 upstream commits, all native runtime or build scripting, taken as a block: the Starboard webOS port under `cobalt-platform/webos/arm/`, 19 Cobalt patches (lifecycle, EGL/SDL surface, SDL input, Starfish player, VP9, AV1, dav1d, Pulse soname and tuning, video fallback, demuxer stop race, hardware capabilities), the SDL-webOS build with the relaunch `SIGCONT` patch, libvpx and dav1d cross-builds, `package-starterless-cobalt.sh` with the ownership normalizer, PR #69 lifecycle and playback fixes, the app-resume fix, "allow screen saver outside playback", the seek-preroll deadlock fix, and PR #75. Web-app side: `adblock-main.js` loses the video-fill IIFE (their screen-saver handling moved native), `shorts-block.js` returns, `adblock-preload.js` and its webpack entry go (see the next rows). Not reviewed hunk by hunk for engine gaps: the changed web files carry no `.closest`, `NodeList.forEach` or injected `<style>`. |
