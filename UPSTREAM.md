...
| AV1 advertised on every webOS set (upstream's capability patch) | **Hidden on `v2` unless `/tmp/ytaf-av1.enable` is 1** (`fork-webos-vp9-only.patch`) | AV1 through the old Starfish decoder path stalls on lg75 (lm21a), and the TV went down once during that stall. Upstream C1 (O20N) owners report AV1 playing, 2160p AV1+Opus on the shared backend. One binary serves both TVs, so the hide also costs lg48 AV1. | Measured on lg75 2026-09-23: AV1 at 2160p plays on the shared backend (Opus streams). Streams YouTube pairs with AAC would still take the old path, where AV1 stalled, so AV1 stays behind the marker until that case is handled or measured; lg48 not measured yet. |
