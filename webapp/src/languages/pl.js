export default {
  ui: {
    title: 'YouTube webOS Cobalt AdFree',
    adblock: 'Blokuj reklamy', sponsoredQrCodeBlock: 'Blokuj sponsorowane wyskakujące kody QR', startupPage: 'Strona startowa', startupPageHome: 'Główna', startupPageSubscriptions: 'Subskrypcje', startupPageShorts: 'Shorts', startupPageLibrary: 'Biblioteka', autoLogin: 'Automatycznie pomijaj wybór konta', sponsorblock: 'Włącz SponsorBlock', ryd: 'Pokaż liczbę negatywnych ocen',
    sponsor: 'Pomiń segmenty sponsorskie', intro: 'Pomiń intro', outro: 'Pomiń zakończenie',
    interaction: 'Pomiń przypomnienia o subskrypcji i polubieniu', selfpromo: 'Pomiń autopromocję',
    musicOfftopic: 'Pomiń muzykę/treści nie na temat', preview: 'Pomiń zapowiedź/podsumowanie',
    filler: 'Pomiń wypełniacze/dygresje', hook: 'Pomiń wstęp/powitanie',
    subtitleOn: 'Napisy: WŁĄCZONE', subtitleOff: 'Napisy: WYŁĄCZONE',
    subtitleUnavailable: 'Napisy niedostępne',
    openHint: 'Naciśnij [ZIELONY], aby otworzyć dodatki YouTube'
  },
  sponsorBlock: {
    sponsor: 'sponsor', intro: 'intro', outro: 'zakończenie', interaction: 'interakcja',
    selfpromo: 'autopromocja', music_offtopic: 'nie na temat', preview: 'zapowiedź', filler: 'wypełniacz',
    hook: 'powitanie', skipping: 'Pomijanie'
  }
};
