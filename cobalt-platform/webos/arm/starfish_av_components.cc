...
class Owner : public player::JobQueue::JobOwner {
 ...
 private:
  ...
  std::unique_ptr<NativeSession> Start(uint64_t generation) {
    ...
    // fork: send the running app's id, as StarfishVideoDecoder does. This fork
    // installs under its own ids, and SAM exports APPID (a package id, so it
    // needs no escaping); the stock id stays the fallback.
    const char* app_id = std::getenv("APPID");
    if (!app_id || !*app_id) app_id = "youtube.leanback.v4";
    const std::string payload = FormatString(
        "{\"args\":[{\"mediaTransportType\":\"BUFFERSTREAM\",\"option\":{"
        "\"appId\":\"%s\",\"windowId\":\"%s\",\"queryPosition\":false,"
        ...
        app_id, window.c_str(), codec_name,
