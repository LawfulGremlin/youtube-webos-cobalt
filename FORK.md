...
(webos-webui's `kb/kbinject`, "Virtual keyboards" below). Two parts, both
fork-owned:
...
- **A keyboard-layout feature in `webapp/src/fork/keyboard-layout.mjs`.**
  ...
  replaced by their standalone accent (`^` and `~` included — carried back
  from webos-webui's twin generator 2026-09-02; before that GB/PL/LT/RO/SI/
  BA/RS/AL/IS typed Cobalt's US character on those keys) and non-printables
  such as AltGr+Space's no-break space dropped. Regenerate after editing the
  layout list in the generator; never edit the output by hand.
...
pointer-socket connection pokes it, and forwards letters only from devices
it classes as keyboards by name) lives in that repo's HANDOFF.md. To this
app it is just a keyboard — same switch, same layout hook — and key hold
length is irrelevant here (20 ms and 60 ms both type; Plex needs 60 ms).
Measured on lg75 2026-09-02: 1.2.0 drops Shift/AltGr on 1–3 keys of every
burst, the `inheritOriginal()` fix above (1.2.1) keeps them. It types by
position from its own evdev-coded copy of the tables (`kb/layouts.json`,
its own libxkbcommon script, the same 32 layout ids), which has to invert
THIS app's tables — a layout or variant change here is a change there too.
As of 2026-09-02 it builds xkb's default variants where this repo takes
nodeadkeys and rs(latin): identical for dk, different at base level for fi,
latam and rs. Characters it cannot place (Danish `<>\`) come back as
skipped, never mistyped, and it clicks the on-screen SEARCH key instead of
sending Enter. It is also how to hardware-verify typing without a keyboard
at the TV: `POST /api/<tv>/type {"text": …}` with the search box open, then
a screenshot — the 2026-09-02 pass on lg75 (debug 0.0.53) got `Ærø Rundt
2026 @home` and `MixedCase {[]}$ (Test) 50% #1!` back verbatim.
